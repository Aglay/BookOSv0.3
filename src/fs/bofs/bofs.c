...
void init_bofs()
{
	...
	bofs_dir_entry_io_buf = kmalloc(SECTOR_SIZE*2);
	bofs_inode_io_buf = kmalloc(SECTOR_SIZE*2);
	bofs_file_io_buf = kmalloc(SECTOR_SIZE*2);
	bofs_io_buf = kmalloc(SECTOR_SIZE*2);
	...
	for(i = 0; i < IO_BUF_NR; i++){
		bofs_io_buf_table[i] = kmalloc(SECTOR_SIZE*2);

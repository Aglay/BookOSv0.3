...
void bofs_exhibit_dir(struct bofs_dir *dir)
{
	...
	printk("dir_entry:%x block_id:%d dir_pos:%d\n",\
		dir->dir_entry, dir->block_id,dir->dir_pos);
}
...
void bofs_close_dir(struct bofs_dir* dir)
{
	...
	if(dir->dir_entry != NULL){
		bofs_close_dir_entry(dir->dir_entry);
	}
	kfree(dir);
}
...
struct bofs_dir *bofs_open_dir(const char *pathname)
{
	...
	if(found == 1){	//fount
		if (record.child_dir->file_type == BOFS_FILE_TYPE_NORMAL) {	///nomal file
			...
			dir = (struct bofs_dir *)kmalloc(sizeof(struct bofs_dir));
			...
			dir->dir_entry = record.child_dir;
...
struct bofs_dir_entry *bofs_read_dir(struct bofs_dir *dir)
{
	...
	/*alloc for dir entry*/
	struct bofs_dir_entry *dir_entry = (struct bofs_dir_entry *)kmalloc(sizeof(struct bofs_dir_entry));
	if(dir_entry == NULL){	
		return NULL;
	}
	/*get a dir entry*/   
	...
	struct bofs_inode inode;
	bofs_load_inode_by_id(&inode, dir->dir_entry->inode);
...
int bofs_search_dir(const char* pathname, struct bofs_dir_search_record *record)
{
	...
	struct bofs_dir_entry *parent_dir = (struct bofs_dir_entry *)kmalloc(sizeof(struct bofs_dir_entry));
	struct bofs_dir_entry *child_dir = (struct bofs_dir_entry *)kmalloc(sizeof(struct bofs_dir_entry));
	...
	for(i = 0; i < deep; i++){
		...
		if(i == 0){	//root dir 
			...
			//get root dir entry
			if(bofs_search_dir_entry(bofs_current_super_block->root_dir->dir_entry, child_dir, name)){	//find
				...
				if(i == deep - 1){	//finally
					...
					record->parent_dir = bofs_current_super_block->root_dir->dir_entry;
...
{
	/*read inode*/
	/*bofs_current_super_block->root_inode = (struct bofs_inode *)kmalloc(sizeof(struct bofs_inode));
	...
	*/
	...
	bofs_current_super_block->root_dir = (struct bofs_dir *)kmalloc(sizeof(struct bofs_dir));
	
	memset(bofs_io_buf, 0, SECTOR_SIZE);
	sector_read(bofs_current_super_block->data_start_lba, bofs_io_buf, 1);
	
	dir_entry = bofs_current_super_block->root_dir->dir_entry;
...
int bofs_make_dir(const char *pathname)
{
	...
	for(i = 0; i < deep; i++){
		...
		if(i == 0){	//root dir , C:/a
			//get root dir entry
			if(bofs_search_dir_entry(bofs_current_super_block->root_dir->dir_entry, &child_dir, name)){	//find, C:/a/b
				memcpy(&parent_dir, &child_dir, sizeof(struct bofs_dir_entry));	//child_dir become parent_dir
				...
				if(i == deep - 1){
					...
					bofs_copy_dir_entry(&dir_1, bofs_current_super_block->root_dir->dir_entry, 0);
					...
					int sync_ret = bofs_sync_dir_entry(bofs_current_super_block->root_dir->dir_entry, &child_dir);
					
					if(sync_ret > 0){	//successed
						if(sync_ret != 2){
							/*change parent dir inode size*/
							bofs_load_inode_by_id(&inode, bofs_current_super_block->root_dir->dir_entry->inode);
			...
			if(bofs_search_dir_entry(bofs_current_super_block->root_dir->dir_entry, &child_dir, name)){	//find, C:/a/b
				...
				if(i == deep - 1){
					...
					*/
					...
					if(bofs_sync_dir_entry(bofs_current_super_block->root_dir->dir_entry, &child_dir)){
						return 0;
					}
				}else{
					memcpy(&parent_dir, &child_dir, sizeof(struct bofs_dir_entry));	//child_dir become parent_dir
